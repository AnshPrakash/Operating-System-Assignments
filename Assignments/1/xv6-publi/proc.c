...
found:
  ...
  *(uint*)sp = (uint)trapret;
  ...
  // Too Complex so I switched to some other strategy
  // for(int i=0;i<NPROC;i++){
  //   if(AllmessageQueues[i].isAcquired==0){
  //     AllmessageQueues[i].isAcquired=1;
  //     AllmessageQueues[i].mid=p->pid;
  //     AllmessageQueues[i].head=0;
  //     AllmessageQueues[i].tail=0;
  //     AllmessageQueues[i].waiting=0;
  //     initlock(&(AllmessageQueues[i].lock), "messageQueue");
  //     break;
  //   }
...
int             
send(int send_id,int recv_id,char* rmessage)
{
  
  rmessage=(char*)rmessage;
  char* buff=rmessage;
  //Making a deep copy
  char msg[MSGSIZE];
  ...
  acquire(&ptable.lock);
  // acquire(&(p->message->lock));
  // cprintf(" Queue proces pid %d Head %d and tail %d \n",p2->pid,p2->message.head,p2->message.tail);
