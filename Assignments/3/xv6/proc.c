...
#define MAX_CONTAINER  21
int no_of_containers = 0;
struct container containers[MAX_CONTAINER];
...
pinit(void)
{
  ...
  initlock(&helperlock.lock,"helperlock");
}
...
void
container_scheduler(void)
{
  if(no_of_containers!=0){
    acquire(&ptable.lock);
    cprintf("Running Container Scheduler\n");
    for(int i =1;i<MAX_CONTAINER;i++){
      if(containers[i].cid!=-1){
          if(containers[i].current_running_process != -1){
              for(int j = (containers[i].current_running_process+1);; j = (j+1)%MAX_NPROC){
                  if(containers[i].ptable.proc[j].pid !=-1 && containers[i].ptable.proc[j].state == WAITING){
                    containers[i].ptable.proc[containers[i].current_running_process].state = WAITING;
                    containers[i].ptable.proc[j].state = RUNNABLE;
                    containers[i].current_running_process = j;
                    cprintf("current_running_process %d ",containers[i].current_running_process);
                    cprintf("Container no. %d \n",containers[i].cid);
                    break;
                  }
              }
          }
          else{
             for(int j = 0; j < MAX_NPROC; j++){
                  if(containers[i].ptable.proc[j].pid !=-1){
                    containers[i].ptable.proc[j].state = RUNNABLE;
                    containers[i].current_running_process = j;
                    cprintf("current_running_process %d \n",containers[i].current_running_process);
                    break;
                  }
              } 
          }
...
create_container(int cid){
  ...
  if (no_of_containers == 0){
    // Initialising the Data Structure for the first time(assume we never delete a container(which is not true))
